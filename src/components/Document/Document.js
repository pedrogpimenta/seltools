...
import { Intent, Button, Classes, Spinner } from "@blueprintjs/core";
...
class Document extends React.Component {
  ...
  handleSaveDocument = (shouldUpdateDate) => {
    ...
    if (this.props.location.search.length > 0) {
      documentObject.parent = new URLSearchParams(
    ...
    if (this.props.location.search.length > 0) {
      const parent = new URLSearchParams(this.props.location.search).get(
        "parent"
      );
        ...
        if (data.document.locked) {
          this.props.dispatch({
            ...
            isLocked:
              ...
              data.document.lockedBy !== this.props.user._id,
  ...
  componentWillUnmount() {
    ...
    if (this.props.socket)
      this.props.socket.removeAllListeners("lock document");
  ...
  componentDidMount() {
    ...
    if (!this.props.match.params.id) {
      ...
      if (!!parent) {
        fetch(`${REACT_APP_SERVER_BASE_URL}/document/${parent}`)
          ...
          .then((data) => {
            ...
            newBreadcrumbs.push({
              icon:
                data.document.type === "folder" ? "folder-open" : "document",
              name: data.document.name,
      ...
      fetch(
        `${REACT_APP_SERVER_BASE_URL}/document/${this.props.match.params.id}/0`
      )
        .then((response) => response.json())
        .then((data) => {
          const LSfiles = data.document.files || [];
          ...
          if (this.props.socket)
            this.props.socket.on("lock document", (userId, documentId) => {
              if (
                documentId === this.props.id &&
                userId !== this.props.user._id
              ) {
                this.handleLockDocument({
  ...
  getMoreFiles = (filesLength) => {
    if (
      this.state.loadingMoreFiles ||
      this.props.files.length >= this.props.filesLength
    )
      return;
    ...
    fetch(
      `${REACT_APP_SERVER_BASE_URL}/document/${this.props.match.params.id}/${this.props.files.length}`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  handleFileInputChange = (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      axios
        .post(`${REACT_APP_SERVER_BASE_URL}/sign_s3`, {
          fileName: fileName,
          fileType: fileType,
        })
        .then((response) => {
          ...
          axios
            .put(signedRequest, files[i], options)
            .then((result) => {
  ...
  handleAddVideoEmbed = (fileIndex, creator) => {
    const url = window.prompt(
      '¿Cuál es la URL del vídeo? Ej: "https://www.youtube.com/watch?v=yfTCbVMmGa0"'
    );
  ...
  renderFileButtons = (i, fileIndex) => {
    return (
      ...
      >
        ...
        {(this.props.user.type !== "student"
          ? true
          : this.props.files[i].creator ===
            localStorage.getItem("studentName")) && (
          <Button
            style={{ margin: "0 4px" }}
            intent={
              this.props.files.length > 0 ? Intent.DEFAULT : Intent.PRIMARY
            }
  ...
  handleClickWhenLocked = () => {
    alert(
      "En este momento, tu profe está revisando el documento. Vuelve más tarde ;)"
    );
  };
          ...
          <div className="document">
            ...
            >
              {this.props.files.map((file, i) => {
                if (this.props.user.type === "student" && file.hidden)
                  return null;
                return (
                  <div
                    key={file.id}
                  ...
                  >
                    ...
                    >
                      {file.type && file.type.toLowerCase() === "pdf" && (
                        <Canvas
                          file={file}
                          fileHasRendered={this.fileHasRendered}
                        />
                      )}
                      ...
                      {file.type &&
                        file.type.toLowerCase() === "videoembed" && (
